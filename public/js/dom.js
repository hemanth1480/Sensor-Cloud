...
function downloadData(asId, title) {
    fetch('http://localhost:59/download-data?id=' + asId, {
            method: 'POST',
            headers: {
        ...
        .then(res => {
            if (res.status == 200) {
                window.open('http://localhost:59/downloadData?newid=' + asId + '&title=' + title);
                // fetch('http://localhost:59/downloadData?newid=' + asId + '&title=' + title, {
                //         method: 'POST',
                //         headers: {
                //             'Accept': 'application/json',
                //             'Content-Type': 'application/json'
